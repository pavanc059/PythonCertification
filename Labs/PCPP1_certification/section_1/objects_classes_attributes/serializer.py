...
class Song(ClientObjectBluePrint):
    ...
    def use_product(self, product):
        ...
        product.add_property("playlist", self.playlist)
...
def serialize(object_to_serialize, data_format):
    # get_serializer was renamed to factory_method
...
class CreatorBluePrint(ABC):
    ...
    def register_format(self, data_format):
        pass
    # no need for abstract dunder methods
...
class SerializerCreator(CreatorBluePrint):
    ...
    # rename get_serializer to factory_method
    def factory_method(self, data_format):
        self.data_format = data_format
        product = self.product.get(data_format, None)
        if product is None:
            raise ValueError(f"Sorry, we don't support {data_format} format yet.")
        return product()

    def register_format(self, data_format, product_class):
        self.product[data_format] = product_class
...
#Register the formats to the creator
